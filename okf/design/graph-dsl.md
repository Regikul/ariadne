...
`ingress` и `egress` не являются конструкциями DSL: они выводятся из границы
`loop` автоматически. `feedback` задаётся явно. После исключения feedback-рёбер
весь граф должен быть ацикличен.
